...
Although the official internship period commenced on Thursday $4^{th}$ of January in 2022, we had had our first meeting with the supervisor about a month prior to that. In that meeting, our supervisor introduced the available projects that we would be working on in our training period. He offered us two separate paths we can select to work during our internship period as described in Table \ref{table:familiarize}. Depending on our interests, we chose  `Plan A' to continue with.
...
\textit{Visual Studio 2019} is simply an \ac{ide} owns and developed by Microsoft. It provides necessary facilities for computer programmers to develop software. It consists of a source code editor, build automation tools, debugger and many more features that an \ac{ide} should contain \cite{vs2019}.\\
